...
class AttentionFusion(nn.Module):
    ...
    def forward(self, rgb_feat, thermal_feat):
        ...
        attention_weights = self.attetion(combined)
        return rgb_feat * attention_weights[:,0:1] + thermal_feat * attention_weights[:,1:2]
...
class RTFNet(nn.Module):
    ...
    def __init__(self, n_class):
        ...
        if self.num_resnet_layers == 18:
            resnet_raw_model1 = models.resnet18(weights=True)
            resnet_raw_model2 = models.resnet18(weights=True)
            self.inplanes = 512
        elif self.num_resnet_layers == 34:
            resnet_raw_model1 = models.resnet34(weights=True)
            resnet_raw_model2 = models.resnet34(weights=True)
            self.inplanes = 512
        elif self.num_resnet_layers == 50:
            resnet_raw_model1 = models.resnet50(weights=True)
            resnet_raw_model2 = models.resnet50(weights=True)
            self.inplanes = 2048
        elif self.num_resnet_layers == 101:
            resnet_raw_model1 = models.resnet101(weights=True)
            resnet_raw_model2 = models.resnet101(weights=True)
            self.inplanes = 2048
        elif self.num_resnet_layers == 152:
            resnet_raw_model1 = models.resnet152(weights=True)
            resnet_raw_model2 = models.resnet152(weights=True)
            self.inplanes = 2048
    ...
    def forward(self, input):
        ...
        # rgb = rgb + thermal
        rgb = self.fusion1(rgb, thermal)  # 融合一次
